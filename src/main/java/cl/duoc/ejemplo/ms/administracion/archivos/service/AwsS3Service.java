...
public class AwsS3Service {
	 ...
	 * @param prefix Prefijo opcional para filtrar (ej. YYYYMM/)
	...
	public void uploadBytes(String bucket, String key, byte[] content, String contentType) {
		if (content == null || content.length == 0) {
			throw new InvalidFileException("El contenido está vacío");
		}
		try {
			log.info("Subiendo archivo (bytes): {} al bucket: {}, tamaño: {} bytes", key, bucket, content.length);
			PutObjectRequest putObjectRequest = PutObjectRequest.builder().bucket(bucket).key(key)
